...
using Invoice.TW.Controllers;
using PKLib_Method.Methods;

public partial class myInvoice_Extend_FeeExpStep1 : SecurityIn
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth_User("1121", out ErrMsg) == false)
                {
    ...
    private void LookupData_Detail(string startDate, string endDate)
    {
        ...
        {
            //----- 原始資料:取得所有資料 -----
            var query = _data.Get_FeeData(startDate.ToDateString("yyyyMMdd"), endDate.ToDateString("yyyyMMdd")
                , null, out ErrMsg);
            ...
            //next button
            ph_Next.Visible = query.Rows.Count > 0;
    ...
    protected void btn_Next_Click(object sender, EventArgs e)
    {
        ...
        //----- 宣告:資料參數 -----
        InvoiceRepository _data = new InvoiceRepository();
        if (!_data.Create_FeeData(_traceID, _payWho, _payAcc1, _payAcc2, cbxGroup, out ErrMsg))
        {
            lt_ShowMsg.Text = ErrMsg;
            ph_ErrMessage.Visible = true;
            return;
        ...
        //導至下一步
        Response.Redirect("{0}/FeeExpStep2.aspx?id={1}".FormatThis(FuncPath(), _traceID));
    ...
    {
        return "{0}myTWInvoice".FormatThis(fn_Params.WebUrl);
    }
