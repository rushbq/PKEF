...
public partial class OP_HelpSearch : SecurityIn
{
    //回覆權限
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 問題類型
                if (fn_Extensions.Menu_OPClass(this.ddl_Req_Class, Request.QueryString["ReqClass"], true, new List<string> { "登記問題類型" }, out ErrMsg) == false)
                {
                    this.ddl_Req_Class.Items.Insert(0, new ListItem("選單產生失敗", ""));
                }

                //[取得/檢查參數] - 處理狀態
                if (fn_Extensions.Menu_OPClass(this.ddl_Help_Status, Request.QueryString["HelpStatus"], true, new List<string> { "處理狀態" }, out ErrMsg) == false)
                {
    ...
    private void LookupDataList(int page)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url
            this.ViewState["Page_LinkStr"] = Application["WebUrl"] + "myOPHelp/OP_HelpSearch.aspx?func=sh";
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    SBSql.AppendLine(" FROM OP_Help_Attach WHERE (TraceID = @Param_ID) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    ...
                    //[SQL] - 刪除資料
                    SBSql.Clear();
                    SBSql.AppendLine(" DELETE FROM OP_Help_Attach WHERE (TraceID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM OP_Help_CC WHERE (TraceID = @Param_ID) ");
                    SBSql.AppendLine(" DELETE FROM OP_Help WHERE (TraceID = @Param_ID) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("Param_ID", Get_ID);
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("OP_HelpSearch.aspx?func=sh");
    ...
    protected void btn_Export_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.Append(" FROM OP_Help Main");
                SBSql.Append("  INNER JOIN OP_ParamClass ReqClass ON Main.Req_Class = ReqClass.Class_ID");
                SBSql.Append("  INNER JOIN OP_ParamClass HelpStatus ON Main.Help_Status = HelpStatus.Class_ID");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    fn_CustomUI.ExportExcel(
                        DT
                        , "{0}-需求明細.xlsx".FormatThis(DateTime.Now.ToShortDateString().ToDateString("yyyyMMdd")));
                }
            }
        }
        catch (Exception)
