...
public partial class mySHInvoice_Step2 : SecurityIn
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth_User("1111", out ErrMsg) == false)
                {
    ...
    private void LookupData()
    {
        ...
        var query = _getData.Take(1)
            .Select(fld => new
            {
                ...
                erp_sDate = fld.erp_sDate,
                erp_eDate = fld.erp_eDate
        ...
        {
            CustomExtension.AlertMsg("重複轉入,請重新確認", "{0}mySHInvoice/List.aspx".FormatThis(fn_Params.WebUrl));
            return;
        }
        ...
        //search date between
        lt_date.Text = "{0} ~ {1}".FormatThis(query.erp_sDate, query.erp_eDate);
    ...
    protected void lbtn_ReNew_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("{0}mySHInvoice/Step1.aspx".FormatThis(fn_Params.WebUrl));
            }
    ...
    protected void btn_Next_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Response.Redirect("{0}mySHInvoice/Step3.aspx?dataID={1}&type={2}".FormatThis(fn_Params.WebUrl, Req_DataID, hf_DataType.Value));
            }
        }
        catch (Exception)
        {
    ...
    public string PageUrl
    {
        get
        {
            return "{0}mySHInvoice/Step2.aspx?dataID={1}".FormatThis(fn_Params.WebUrl, Req_DataID);
        }
        set
        {
            this._PageUrl = value;
