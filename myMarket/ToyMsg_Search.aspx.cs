...
public partial class ToyMsg_Search : SecurityIn
{
    //回覆權限
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 訊息類別
                if (fn_Extensions.Menu_ToyInqClass(this.ddl_Req_Class, Request.QueryString["ReqClass"], true, out ErrMsg) == false)
                {
                    this.ddl_Req_Class.Items.Insert(0, new ListItem("選單產生失敗", ""));
                }

                //[取得/檢查參數] - 處理狀態
                if (fn_Extensions.Menu_ToyInqStatus(this.ddl_Status, Request.QueryString["ReqStatus"], true, out ErrMsg) == false)
                {
    ...
    private void LookupDataList(int page)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url
            this.ViewState["Page_LinkStr"] = Application["WebUrl"] + "myMarket/ToyMsg_Search.aspx?t=" + Param_Type;
            ...
            //[SQL] - 取得資料
            using (DataTable DT = dbConn.LookupDTwithPage(cmd, cmdTotalCnt, dbConn.DBS.Science, out TotalCnt, out ErrMsg))
            {
                if (DT.Rows.Count == 0)
                {
                    //判斷是否為頁碼過大, 帶往最後一頁
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("ToyMsg_Search.aspx?t=" + Param_Type);
