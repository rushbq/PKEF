...
public partial class ToyMsg_Edit : SecurityIn
{
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.Science, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        //取得狀態
                        this.hf_Current_Status.Value = myStatus;
                        string setCss;
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.Science, out ErrMsg))
                {
                    if (DT.Rows.Count > 0)
                    {
                        ...
                        for (int row = 0; row < DT.Rows.Count; row++)
                        {
                            ...
                            itemHtml.Append("<a href=\"javascript:Delete_Item('{0}{1}');\" class=\"btn btn-success\" title=\"{3}\">{2}&nbsp;<span class=\"glyphicon glyphicon-trash\"></span></a>"
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                if (dbConn.ExecuteSql(cmd, dbConn.DBS.Science, out ErrMsg) == false)
                {
    ...
    protected void btn_Reply_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            myMailBody.Append("</tr>");
            myMailBody.Append("<tr>");
            myMailBody.Append("    <td width=\"15%\" align=\"right\" bgcolor=\"#9f9f9f\" style=\"color: #ffffff\">姓</td>");
            myMailBody.Append("    <td width=\"35%\">{0}</td>".FormatThis(this.modal_FirstName.Text));
            myMailBody.Append("    <td width=\"15%\" align=\"right\" bgcolor=\"#9f9f9f\" style=\"color: #ffffff\">名</td>");
            myMailBody.Append("    <td width=\"35%\">{0}</td>".FormatThis(this.modal_LastName.Text));
            myMailBody.Append("</tr>");
            myMailBody.Append("<tr>");
            ...
            //發信
            sendMail_transfer(
                "{0}-科學玩具網站留言-客戶詳細資料 #{1}".FormatThis(DateTime.Now.ToShortDateString().ToDateString("yyyyMMdd"), this.lt_TraceID.Text)
                , myMailBody
                , this.lt_ReplyMailSender.Text
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //轉到檢視頁
                Response.Redirect("ToyMsg_Edit.aspx?DataID={0}".FormatThis(Server.UrlEncode(Cryptograph.MD5Encrypt(Param_thisID, DesKey))));
            }
    ...
    protected void btn_Finish_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //轉到檢視頁
                Response.Redirect("ToyMsg_View.aspx?DataID={0}".FormatThis(Server.UrlEncode(Cryptograph.MD5Encrypt(Param_thisID, DesKey))));
            }
    ...
    private bool SendMail(string Subject, StringBuilder mailBody, string emailFrom, string emailTo, List<string> ForwardTo, out string ErrMsg)
    {
        ...
        //[設定參數] - 來源程式/功能
        fn_Mail.FromFunc = "PKScience, Inquiry回覆";

        //[設定參數] - 寄件人
        fn_Mail.Sender = emailFrom;

        //[設定參數] - 寄件人顯示名稱
        fn_Mail.SenderName = "Pro'sKit Professional Science Kits";
    ...
    public string PageUrl
    {
        get
        {
            return string.Format(@"ToyMsg_Edit.aspx?DataID={0}"
