...
public class ADService
{
    ...
    public class LookupLDAP
    {
        ...
        public LookupLDAP(string GUID, string SID, string DisplayName, string AccountName, string Email, string Desc)
        {
            this._GUID = GUID;
            this._SID = SID;
        ...
        public LookupLDAP(string GUID, string SID, string DisplayName, string AccountName, string Email, string Desc
            , string Department, int userAccountControl)
        {
            this._GUID = GUID;
            this._SID = SID;
            this._DisplayName = DisplayName;
    ...
    public static List<LookupLDAP> ListGroups(string LPath)
    {
        try
        {
            ...
            //顯示欄位 (顯示名稱, 登入帳號, 完整路徑)
            string[] loadProps = new string[] { "cn", "samaccountname", "objectGUID", "objectSid", "description" };
            ...
            using (DirectorySearcher deSearch = new DirectorySearcher(de, "(objectClass=group)", loadProps))
            {
                ...
                foreach (SearchResult result in results)
                {
                    ...
                    using (DirectoryEntry entry = result.GetDirectoryEntry())
                    {
                        ...
                        //Description(可能有Null值)
                        string myDesc = "";
                        if (entry.Properties["description"].Value != null)
                        {
                            myDesc = result.Properties["description"][0].ToString();
                        }
    ...
    /// <summary>
    /// 取得使用者/群組屬性[0: SID, 1:name, 2:sAMAccountName, 3.Guid, 4:ObjectType, 5:Description]
    /// </summary>
    /// <param name="pGUID">GUID</param>
    /// <returns>陣列</returns>
    public static StringCollection getAttributesFromGUID(String pGUID)
    {
        try
        {
            ...
            if (groupResult != null)
            {
                ...
                //description
                string myDesc = "";
                if (groupEntry.Properties["description"].Value != null)
                {
                    myDesc = groupEntry.Properties["description"][0].ToString();
                }
                result.Add(myDesc);
            }
            else
