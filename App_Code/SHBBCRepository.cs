    ...
    public class SHBBCRepository
    {
        ...
        public IQueryable<RefColumn> GetDetailList(string parentID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SHBBC_ImportData_DT WITH(NOLOCK)");
        ...
        public IQueryable<RefColumn> GetTempDTGroup(string parentID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SHBBC_ImportData_TempDT WITH(NOLOCK)");
        ...
        public IQueryable<RefLog> GetLogList(string dataID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" SELECT *");
                sql.AppendLine(" FROM SHBBC_ImportData_Log WITH(NOLOCK)");
        ...
        public IQueryable<CustRef> GetRefCust(Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SHBBC_RefTable");
                ...
                foreach (var val in queryVals)
                {
                    ...
                    switch (dataType)
                    {
                        ...
                        case "3":
                            ...
                            /*
                             * [判斷商城]
                             * 1:京東POP / 2:天貓 / 3:京東VC / 4:eService / 5:唯品會 / 6:京東廠送 / 999:通用版
                             */
                            switch (mallID)
                            {
                                case "1":
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    switch (promoType)
                    {
                        case "1":
                            ...
                            sql.AppendLine(" FROM SHBBC_Promo Base");
                            ...
                            sql.AppendLine(" FROM SHBBC_ImportData_DT, TblPromo");
                            ...
                            sql.AppendLine(" FROM SHBBC_ImportData_DT DT");
                            sql.AppendLine("  INNER JOIN TblGroup ON DT.OrderID = TblGroup.OrderID");
                            sql.AppendLine("  INNER JOIN SHBBC_Promo_DT GiftDT ON TblGroup.PromoID = GiftDT.Parent_ID");
                        ...
                        default:
                            ...
                            sql.AppendLine(" FROM SHBBC_ImportData_DT");
        ...
        public IQueryable<PromoBase> GetPromoConfig(Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" , (SELECT COUNT(*) FROM SHBBC_Promo_DT WHERE (Parent_ID = Base.Data_ID)) AS ChildCnt");
                sql.AppendLine(" FROM SHBBC_Promo Base");
                sql.AppendLine("  INNER JOIN SHBBC_RefClass Cls ON Base.MallID = Cls.Class_ID");
                sql.AppendLine(" WHERE (1=1)");
        ...
        public IQueryable<PromoDT> GetPromoConfigDetail(string parentID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SHBBC_Promo_DT");
                sql.AppendLine(" WHERE (Parent_ID = @ParentID)");
        ...
        public IQueryable<Shipment> GetShipmentData(Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM [PKEF].[dbo].[SHBBC_InvoiceItem] InvItem");
                sql.AppendLine(" )");
                ...
                sql.AppendLine(" FROM [PKEF].dbo.SHBBC_ImportData AS Base");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_ImportData_DT AS DT ON Base.Data_ID = DT.Parent_ID");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_RefClass Cls ON Base.MallID = Cls.Class_ID");
                ...
                sql.AppendLine(" 	FROM [PKEF].dbo.SHBBC_ShipExport Rel");
                sql.AppendLine(" 	 INNER JOIN [PKEF].dbo.SHBBC_ShipExport_DT RelDT ON Rel.Data_ID = RelDT.Parent_ID");
                sql.AppendLine("  ))");
        ...
        public IQueryable<Shipment> GetShipmentHistoryDetail(Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" SELECT * FROM [PKEF].[dbo].[SHBBC_ShipExport_DT]");
                ...
                sql.AppendLine("  , InvItem.InvAddrInfo");
                sql.AppendLine("  , InvItem.InvBankInfo");
                sql.AppendLine(" FROM [PKEF].dbo.SHBBC_ShipExport AS Rel");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_ShipExport_DT AS RelDT ON Rel.Data_ID = RelDT.Parent_ID");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_ImportData AS Base ON Base.Data_ID = RelDT.SrcParentID");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_ImportData_DT AS DT ON Base.Data_ID = DT.Parent_ID AND DT.Data_ID = RelDT.SrcDataID");
                sql.AppendLine("  INNER JOIN [PKEF].dbo.SHBBC_RefClass Cls ON Base.MallID = Cls.Class_ID");
                sql.AppendLine("  INNER JOIN TblERP ON DT.OrderID = TblERP.OrderID AND DT.ERP_ModelNo = TblERP.ModelNo AND DT.ProdID = TblERP.CustModelNo");
                sql.AppendLine("  LEFT JOIN TblInvItem InvItem ON DT.OrderID = InvItem.OrderID AND Base.TraceID = InvItem.TraceID AND InvItem.RowRank = 1");
                sql.AppendLine(" WHERE (DT.IsPass = 'Y')");
        ...
        public IQueryable<StockData> GetStockData(Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine("  FROM [PKEF].dbo.SHBBC_StockData Base");
                sql.AppendLine("   INNER JOIN [PKEF].dbo.SHBBC_StockData_DT DT ON Base.Data_ID = DT.Parent_ID");
        ...
        public IQueryable<StockImportDataDT> GetStockImportDetail(string parentID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" SELECT *");
                sql.AppendLine(" FROM SHBBC_StockData_DT WITH(NOLOCK)");
                sql.AppendLine(" WHERE (Parent_ID = @ParentID)");
                sql.AppendLine(" ORDER BY Data_ID");
        ...
        public IQueryable<RefModel> GetRef_ChkList(out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" 	WHERE (DB = 'SH') AND (ISNULL(MG006, '') = '') AND (MG002 <> MG003)");
                ...
                sql.AppendLine("  INNER JOIN PKEF.dbo.SHBBC_RefClass Cls ON Base.MallID = Cls.Class_ID");
                sql.AppendLine(" WHERE (Base.DB = 'SH') AND (TblGroup.repeatCnt > 1)");
        ...
        public bool Create(ImportData instance)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SHBBC_ImportData( ");
        ...
        public bool Create_Temp(ImportData baseData, IQueryable<RefColumn> query, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SHBBC_ImportData_TempDT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SHBBC_ImportData_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" UPDATE SHBBC_ImportData SET Status = 11, Sheet_Name = @Sheet_Name, Update_Who = @Update_Who, Update_Time = GETDATE() WHERE (Data_ID = @DataID);");
                ...
                foreach (var item in query)
                {
                    if (!string.IsNullOrEmpty(item.ProdID))
                    {
                        ...
                        sql.AppendLine("  FROM SHBBC_ImportData_TempDT ");
                        sql.AppendLine("  WHERE Parent_ID = @DataID ");
                        sql.AppendLine(" )");

                        sql.AppendLine(" INSERT INTO SHBBC_ImportData_TempDT( ");
                ...
                //處理合併欄位, 總金額為 0的欄位
                sql.AppendLine(" UPDATE SHBBC_ImportData_TempDT SET TotalPrice = ISNULL((");
                sql.AppendLine("  SELECT TOP 1 Totalprice FROM SHBBC_ImportData_TempDT Ref WHERE (Ref.Parent_ID = @DataID)");
                sql.AppendLine("    AND (Ref.OrderID = SHBBC_ImportData_TempDT.OrderID) AND (Ref.TotalPrice <> 0)");
                sql.AppendLine(" ), 0)");
        ...
        public bool Create_Log(ImportData baseData, string desc, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" DECLARE @NewID AS INT ");
                sql.AppendLine(" SET @NewID = (");
                sql.AppendLine("  SELECT ISNULL(MAX(Log_ID), 0) + 1 AS NewID FROM SHBBC_ImportData_Log");
                sql.AppendLine(" )");

                sql.AppendLine(" INSERT INTO SHBBC_ImportData_Log( ");
        ...
        public bool Create_EDI(ImportData baseData, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" , LEFT(MallCls.Class_Name, 20) AS XA024"); //來源平台
                ...
                sql.AppendLine(" FROM SHBBC_ImportData Base WITH(NOLOCK)");
                sql.AppendLine("  INNER JOIN SHBBC_ImportData_DT DT WITH(NOLOCK) ON Base.Data_ID = DT.Parent_ID");
                sql.AppendLine("  INNER JOIN SHBBC_RefClass MallCls WITH(NOLOCK) ON Base.MallID = MallCls.Class_ID");
        ...
        public bool Create_Promo(PromoBase instance, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SHBBC_Promo( ");
        ...
        public bool Create_PromoDT(PromoDT inst, string parentID, string updateWho, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SHBBC_Promo SET");
                sql.AppendLine(" Update_Who = @Update_Who, Update_Time = GETDATE()");
                sql.AppendLine(" WHERE (Data_ID = @Parent_ID);");

                sql.AppendLine(" DECLARE @NewID AS INT ");
                sql.AppendLine(" SET @NewID = (");
                sql.AppendLine("  SELECT ISNULL(MAX(Data_ID), 0) + 1 AS NewID FROM SHBBC_Promo_DT WHERE (Parent_ID = @Parent_ID)");
                sql.AppendLine(" );");

                sql.Append(" INSERT INTO SHBBC_Promo_DT( ");
        ...
        public bool Create_TempNewItem(string parentID, string orderID, int itemID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" , RemarkTitle, RemarkID, NickName, BuyRemark, SellRemark");
                sql.AppendLine(" FROM SHBBC_ImportData_TempDT");
                sql.AppendLine("  LEFT JOIN [PKSYS].[dbo].[refCOPMG] Ref ON Ref.Data_ID = @ItemID AND Ref.DB = 'SH'");
        ...
        public bool Create_ShipExport(ShipExport instance, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SHBBC_ShipExport( ");
        ...
        public bool Create_ShipExportDT(IQueryable<ShipExportDT> myData, string parentID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                {
                    sql.AppendLine(" SET @NewID = (");
                    sql.AppendLine("  SELECT ISNULL(MAX(Data_ID), 0) + 1 AS NewID FROM SHBBC_ShipExport_DT WHERE (Parent_ID = @Parent_ID)");
                    sql.AppendLine(" );");

                    sql.AppendLine(" INSERT INTO SHBBC_ShipExport_DT( ");
        ...
        public bool Create_StockData(StockImportData instance, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SHBBC_StockData( ");
        ...
        public bool Create_StockDataDT(StockImportData baseData, IQueryable<StockImportDataDT> query, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SHBBC_StockData_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" UPDATE SHBBC_StockData SET Update_Who = @Update_Who, Update_Time = GETDATE() WHERE (Data_ID = @DataID);");
                ...
                foreach (var item in query)
                {
                    if (!string.IsNullOrWhiteSpace(item.ProdID))
                    {
                        sql.AppendLine(" SET @NewID = (");
                        sql.AppendLine("  SELECT ISNULL(MAX(Data_ID), 0) + 1 AS NewID ");
                        sql.AppendLine("  FROM SHBBC_StockData_DT ");
                        sql.AppendLine("  WHERE Parent_ID = @DataID ");
                        sql.AppendLine(" )");

                        sql.AppendLine(" INSERT INTO SHBBC_StockData_DT( ");
                ...
                //Update 品號
                sql.AppendLine(" UPDATE SHBBC_StockData_DT");
                ...
                sql.AppendLine(" AND (SHBBC_StockData_DT.Parent_ID = @DataID);");
                //Update IsPass
                sql.AppendLine(" UPDATE SHBBC_StockData_DT SET IsPass = 'N', doWhat = '查無ERP品號(客戶品號資料檔)' WHERE (Parent_ID = @DataID) AND (ERP_ModelNo IS NULL);");
        ...
        public bool Create_RefModels(IQueryable<RefModel> instance, string custID, Int32 mallID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM refCOPMG WHERE (MG001 = @CustID) AND (MallID = @MallID) AND (DB = 'SH')");
                //sql.AppendLine(" DECLARE @NewID AS INT");
        ...
        public bool Delete(string dataID)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SHBBC_InvoiceItem");
                sql.AppendLine(" WHERE EXISTS (");
                sql.AppendLine("    SELECT *");
                sql.AppendLine("    FROM SHBBC_ImportData Base");
                sql.AppendLine("     INNER JOIN SHBBC_ImportData_DT DT ON Base.Data_ID = DT.Parent_ID");
                sql.AppendLine("    WHERE (Base.Data_ID = @DataID)");
                sql.AppendLine("     AND (SHBBC_InvoiceItem.TraceID = Base.TraceID) AND (SHBBC_InvoiceItem.OrderID = DT.OrderID)");
                sql.AppendLine(" );");

                sql.AppendLine(" DELETE FROM SHBBC_ImportData_TempDT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SHBBC_ImportData_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SHBBC_ImportData WHERE (Data_ID = @DataID);");
        ...
        public bool Delete_Promo(string dataID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SHBBC_Promo_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SHBBC_Promo WHERE (Data_ID = @DataID);");
        ...
        public bool Delete_PromoDT(string parentID, string dataID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SHBBC_Promo_DT WHERE (Parent_ID = @Parent_ID) AND (Data_ID = @Data_ID)");
        ...
        public bool Check_Step1(ImportData baseData, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                switch (baseData.MallID)
                {
                    ...
                    case 999:
                        //唯品會, VC工業品, 通用
                        sql.AppendLine(" INSERT INTO SHBBC_ImportData_DT (");
        ...
        /// <remarks>
        /// Step2 時執行
        /// SHBBC_Import_DT 與其他不同Parent_ID的資料比對, 判斷是否有重複的OrderID + ProdID, 若有重複則 IsPass = 'N',doWhat = '重複的單號'
        /// </remarks>
        public bool Check_Step2(string dataID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine("     SELECT Base.Data_ID");
                sql.AppendLine("     FROM SHBBC_ImportData_DT Base");
                sql.AppendLine("     WHERE (Base.Parent_ID = @DataID)");
                sql.AppendLine("      AND EXISTS (");
                sql.AppendLine("       SELECT OrderID FROM SHBBC_ImportData_DT Chk");
        ...
        public bool Check_Step3(string dataID, out string ErrMsg)
        {
            ...
            //***** 取得ERP資料 - 報價, 並回寫至 SHBBC_ImportData_DT ***** 
            List<RefColumn> dataList = new List<RefColumn>();
            var data = new RefColumn();
            ...
            switch (MallID)
            {
                case "3":
                    //VC(抓客戶出貨庫別)
                    //----- 更新 SHBBC_ImportData_DT, 庫存(庫存-預計銷-安全存量), myPrc_GetStock -----
        ...
        private bool Update_Price(string dataID, IQueryable<RefColumn> query)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //將價格為 0 的資料Update 狀態
                sql.AppendLine(" UPDATE SHBBC_ImportData_DT SET IsPass = 'N', doWhat = N'查無ERP價格'");
        ...
        private bool Update_Stock(string dataID, IQueryable<RefColumn> query)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" UPDATE SHBBC_ImportData_DT SET StockStatus = '1'");
                    ...
                    using (SqlCommand cmdDT = new SqlCommand())
                    {
                        for (int row = 0; row < DT.Rows.Count; row++)
                        {
                            ...
                            if (CntPrice < 0)
                            {
                                ...
                                dataSQL.AppendLine("  SELECT COUNT(*) FROM SHBBC_ImportData_DT WHERE (Parent_ID = @DataID) AND (OrderID = N'{0}') AND (StockStatus = '1')".FormatThis(OrderID));
                                ...
                                dataSQL.AppendLine("  FROM SHBBC_ImportData_DT ");
                                ...
                                dataSQL.AppendLine(" INSERT INTO SHBBC_ImportData_DT( ");
                            ...
                            {
                                //--- 更新價格 ---
                                dataSQL.AppendLine(" UPDATE SHBBC_ImportData_DT");
                        ...
                        //--- 更新主檔狀態 ---
                        dataSQL.AppendLine(" UPDATE SHBBC_ImportData SET Status = 12 WHERE (Data_ID = @DataID);");
        ...
        public bool Update_Info(string dataID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine("  SELECT TOP 1 Rel.ShipWho FROM SHBBC_ImportData_DT Rel");
                sql.AppendLine("  WHERE (Rel.Parent_ID = @DataID)");
                sql.AppendLine("  AND (Rel.ShipWho <> '')");
                sql.AppendLine("  AND (Rel.OrderID = SHBBC_ImportData_DT.OrderID)");
                sql.AppendLine(" )");
                sql.AppendLine(" , ShipAddr = (");
                sql.AppendLine("  SELECT TOP 1 Rel.ShipAddr FROM SHBBC_ImportData_DT Rel");
                sql.AppendLine("  WHERE (Rel.Parent_ID = @DataID)");
                sql.AppendLine("  AND (Rel.ShipAddr <> '')");
                sql.AppendLine("  AND (Rel.OrderID = SHBBC_ImportData_DT.OrderID)");
                sql.AppendLine(" )");
                sql.AppendLine(" , ShipTel = (");
                sql.AppendLine("  SELECT TOP 1 Rel.ShipTel FROM SHBBC_ImportData_DT Rel");
                sql.AppendLine("  WHERE (Rel.Parent_ID = @DataID)");
                sql.AppendLine("  AND (Rel.ShipTel <> '')");
                sql.AppendLine("  AND (Rel.OrderID = SHBBC_ImportData_DT.OrderID)");
                sql.AppendLine(" )");
                sql.AppendLine(" , ShipmentNo = (");
                sql.AppendLine("  SELECT TOP 1 Rel.ShipmentNo FROM SHBBC_ImportData_DT Rel");
                sql.AppendLine("  WHERE (Rel.Parent_ID = @DataID)");
                sql.AppendLine("  AND (Rel.ShipmentNo <> '')");
                sql.AppendLine("  AND (Rel.OrderID = SHBBC_ImportData_DT.OrderID)");
                sql.AppendLine(" )");
                sql.AppendLine(" , RemarkTitle = (");
                sql.AppendLine(" SELECT TOP 1 Rel.RemarkTitle FROM SHBBC_ImportData_DT Rel");
                sql.AppendLine(" WHERE (Rel.Parent_ID = @DataID)");
        ...
        public bool Update_Status(string dataID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SHBBC_ImportData SET Status = 13, Import_Time = GETDATE() WHERE (Data_ID = @DataID)");
        ...
        public bool Update_PromoData(PromoBase inst, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SHBBC_Promo SET PromoName = @PromoName, MallID = @MallID");
        ...
        public bool Update_TempDT(List<RefColumn> dataItems, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                foreach (var item in dataItems)
                {
                    ...
                    //SQL
                    sql.AppendLine(" UPDATE SHBBC_ImportData_TempDT");
        ...
        public bool Update_StockData(StockImportData inst, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SHBBC_StockData SET CustID = @CustID, Upload_File = @Upload_File");
                sql.AppendLine(" , Update_Who = @Update_Who, Update_Time = GETDATE()");
        ...
        private DataTable LookupRawData(Dictionary<int, string> search)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine("   , (SELECT Account_Name + ' (' + Display_Name + ')' FROM PKSYS.dbo.User_Profile WHERE (Guid = Base.Create_Who)) AS Create_Name");
                sql.AppendLine("   , (SELECT Account_Name + ' (' + Display_Name + ')' FROM PKSYS.dbo.User_Profile WHERE (Guid = Base.Update_Who)) AS Update_Name");
                sql.AppendLine("   , (SELECT COUNT(*) FROM SHBBC_ImportData_Log WHERE (Data_ID = Base.Data_ID)) AS LogCnt");
                sql.AppendLine(" FROM SHBBC_ImportData Base");
                sql.AppendLine("  INNER JOIN SHBBC_RefClass Cls ON Base.MallID = Cls.Class_ID");
                sql.AppendLine("  LEFT JOIN SHBBC_RefClass ClsSt ON Base.Status = ClsSt.Class_ID");
                sql.AppendLine(" WHERE (1 = 1) ");
                        ...
                        switch (item.Key)
                        {
                            ...
                            case (int)mySearch.Keyword:
                                if (!string.IsNullOrEmpty(item.Value))
                                {
                                    ...
                                    //原始單號
                                    sql.Append("    OR (");
                                    sql.Append("      Base.Data_ID IN (SELECT Parent_ID FROM SHBBC_ImportData_DT WHERE ((UPPER(RTRIM(OrderID)) LIKE '%' + UPPER(@Keyword) + '%') OR (UPPER(ShipmentNo) LIKE '%' + UPPER(@Keyword) + '%') OR (UPPER(ShipTel) LIKE '%' + UPPER(@Keyword) + '%')))");
                                    sql.Append("    )");
                                    sql.Append(" )");
        ...
        private DataTable LookupRawData_Status(myClass cls)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SHBBC_RefClass WITH(NOLOCK)");
