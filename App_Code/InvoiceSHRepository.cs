 ...
 * 3. 轉出匯款資料(SH)
 */
namespace Invoice.SH.Controllers
{
    /// <summary>
    /// 查詢參數
    /// </summary>
    ...
    public class InvoiceRepository
    {
        ...
        public DataTable Get_FeeData(string startDate, string endDate, Dictionary<string, string> search, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                ;WITH TblErp AS (
	                ...
	                WHERE (Corp.Display = 'Y') AND (Corp.Corp_UID = 3)
                )
                ...
                /* SH:收款人名称, 收款人账号, 接收行, 开户行名称, 收款人开户行*/
                , TblErp.cn_AccName, TblErp.cn_Account, TblErp.cn_BankID, TblErp.cn_BankName, TblErp.cn_BankType
                , ROW_NUMBER() OVER(ORDER BY Base.TF001, Base.TF002) AS SerialNo
                FROM [SHPK2].dbo.PCMTF Base
            ...
            , out string ErrMsg)
        {
            ...
            {
                string sql = @"
                DELETE FROM [PKEF].dbo.SH_Fee_Export WHERE (TraceID = @TraceID)
                ;WITH TblErp AS (
	                /* CorpUID = 3 = SH */
	                SELECT 
		                RTRIM(ERP.MA001) ERP_SupID, RTRIM(ERP.MA002) ERP_SupName
                ...
                INSERT INTO [PKEF].dbo.SH_Fee_Export (
                ...
                , TblErp.cn_Account, TblErp.cn_AccName, TblErp.cn_BankName, TblErp.cn_BankID, TblErp.cn_BankType
        ...
        public bool Delete_FeeData(string dataID)
        {
            ...
            {
                //----- SQL 語法 -----
                sql.AppendLine(" DELETE FROM SH_Fee_Export WHERE (TraceID = @TraceID)");
