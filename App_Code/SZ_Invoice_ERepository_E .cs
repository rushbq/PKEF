...
/*
 * 主要資料庫:ProUnion
 * 資料DB:PKEF.SZ_Invoice_ImportData
 * 深圳開票-電子發票匯入
 */
namespace SZ_Invoice_E.Controllers
{
    /// <summary>
    /// 查詢參數
    /// </summary>
    ...
    public class SZ_Invoice_ERepository
    {
        ...
        public IQueryable<RefColumn> GetDetailList(string parentID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SZ_Invoice_ImportData_DT WITH(NOLOCK)");
        ...
        public IQueryable<RefLog> GetLogList(string dataID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" SELECT *");
                sql.AppendLine(" FROM SZ_Invoice_ImportData_Log WITH(NOLOCK)");
        ...
        public IQueryable<ERPData> GetERPData(Dictionary<int, string> search)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SZ_Invoice_ImportData_DT DT");
                sql.AppendLine("  INNER JOIN [ProUnion].dbo.ACRTA WITH(NOLOCK) ON (ACRTA.TA001 + '-' + ACRTA.TA002) = DT.Erp_AR_ID COLLATE Chinese_Taiwan_Stroke_BIN");
        ...
        public bool Create(ImportData instance, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SZ_Invoice_ImportData( ");
        ...
        public bool Create_Temp(ImportData baseData, IQueryable<RefColumn> query, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" DELETE FROM SZ_Invoice_ImportData_TempDT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SZ_Invoice_ImportData_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" UPDATE SZ_Invoice_ImportData SET Status = 11, Sheet_Name = @Sheet_Name, Update_Who = @Update_Who, Update_Time = GETDATE() WHERE (Data_ID = @DataID);");
                ...
                foreach (var item in query)
                {
                    if (!string.IsNullOrEmpty(item.InvoiceNo))
                    {
                        ...
                        sql.AppendLine("  FROM SZ_Invoice_ImportData_TempDT ");
                        sql.AppendLine("  WHERE Parent_ID = @DataID ");
                        sql.AppendLine(" )");

                        sql.AppendLine(" INSERT INTO SZ_Invoice_ImportData_TempDT( ");
                ...
                //Update結帳單號/銷貨單號
                sql.AppendLine(" UPDATE SZ_Invoice_ImportData_TempDT");
                ...
                sql.AppendLine("     FROM SZ_Invoice_ImportData_TempDT AS DT");
                sql.AppendLine("      INNER JOIN [ProUnion].dbo.COPTC ON COPTC.TC012 = DT.OrderID COLLATE Chinese_Taiwan_Stroke_BIN");
                sql.AppendLine("      INNER JOIN [ProUnion].dbo.COPTH ON COPTC.TC001 = COPTH.TH014 AND COPTC.TC002 = COPTH.TH015");
                sql.AppendLine("      INNER JOIN [ProUnion].dbo.ACRTB WITH(NOLOCK) ON ACRTB.TB005 = COPTH.TH001 AND ACRTB.TB006 = COPTH.TH002");
                ...
                sql.AppendLine(" WHERE (SZ_Invoice_ImportData_TempDT.OrderID = TblBase.OrderID)");
                sql.AppendLine(" AND (SZ_Invoice_ImportData_TempDT.Parent_ID = @DataID)");
        ...
        public bool Create_Log(ImportData baseData, string desc, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" DECLARE @NewID AS INT ");
                sql.AppendLine(" SET @NewID = (");
                sql.AppendLine("  SELECT ISNULL(MAX(Log_ID), 0) + 1 AS NewID FROM SZ_Invoice_ImportData_Log");
                sql.AppendLine(" )");

                sql.AppendLine(" INSERT INTO SZ_Invoice_ImportData_Log( ");
        ...
        public bool Delete(string dataID)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" DELETE FROM SZ_Invoice_ImportData_TempDT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SZ_Invoice_ImportData_DT WHERE (Parent_ID = @DataID);");
                sql.AppendLine(" DELETE FROM SZ_Invoice_ImportData WHERE (Data_ID = @DataID);");
        ...
        public bool Check_Step1(string dataID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" INSERT INTO SZ_Invoice_ImportData_DT (");
                ...
                sql.AppendLine(" FROM SZ_Invoice_ImportData_TempDT Tmp");
        ...
        public bool Check_Step2(string dataID, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SZ_Invoice_ImportData_DT");
                sql.AppendLine(" SET IsPass = 'N', doWhat = '重複的單號及發票號'");
                sql.AppendLine(" WHERE (Parent_ID = @DataID)");
                sql.AppendLine("  AND (Data_ID IN (");
                sql.AppendLine("     SELECT Base.Data_ID");
                sql.AppendLine("     FROM SZ_Invoice_ImportData_DT Base");
                sql.AppendLine("     WHERE (Base.Parent_ID = @DataID)");
                sql.AppendLine("      AND EXISTS (");
                sql.AppendLine("       SELECT OrderID FROM SZ_Invoice_ImportData_DT Chk");
        ...
        public bool UpdateInvNo(string dataID, out string ErrMsg)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM [PKEF].dbo.SZ_Invoice_ImportData_DT AS DT");
        ...
        public bool Update_Status(ImportData baseData, out string ErrMsg)
        {
            ...
            {
                //----- SQL 查詢語法 -----
                sql.AppendLine(" UPDATE SZ_Invoice_ImportData SET Status = 12, Import_Time = GETDATE(), Import_Who = @Who WHERE (Data_ID = @DataID)");
        ...
        private DataTable LookupRawData(Dictionary<int, string> search)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine("   , (SELECT COUNT(*) FROM SZ_Invoice_ImportData_Log WHERE (Data_ID = Base.Data_ID)) AS LogCnt");
                sql.AppendLine(" FROM SZ_Invoice_ImportData Base");
                sql.AppendLine("  LEFT JOIN SZ_Invoice_RefClass ClsSt ON Base.Status = ClsSt.Class_ID");
                sql.AppendLine(" WHERE (1 = 1) ");
                        ...
                        switch (item.Key)
                        {
                            ...
                            case (int)mySearch.Keyword:
                                if (!string.IsNullOrEmpty(item.Value))
                                {
                                    ...
                                    //原始單號,發票號
                                    sql.Append("    OR (");
                                    sql.Append("      Base.Data_ID IN (SELECT Parent_ID FROM SZ_Invoice_ImportData_DT WHERE (UPPER(RTRIM(OrderID)) LIKE '%' + UPPER(@Keyword) + '%') OR (UPPER(RTRIM(InvoiceNo)) LIKE '%' + UPPER(@Keyword) + '%'))");
                                    sql.Append("    )");
                                    sql.Append(" )");
        ...
        private DataTable LookupRawData_Status(myClass cls)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql.AppendLine(" FROM SZ_Invoice_RefClass WITH(NOLOCK)");
