<%@ Page Title="報價單匯入 | 匯入記錄" Language="C#" MasterPageFile="~/Site_S_UI.master" AutoEventWireup="true" CodeFile="ImportList.aspx.cs" Inherits="myErpPriceData_ImportList" %>
            ...
            <div class="item">
                <div class="ui small breadcrumb">
                    ...
                    <h5 class="active section">
                        匯入記錄&nbsp;&nbsp;(<span class="red-text text-darken-2"><%=Req_DBS %></span>)
                    </h5>
                </div>
            </div>
            <div class="right menu">
                <a href="ImportStep1.aspx?dbs=<%=Req_DBS %>" class="item"><i class="plus icon"></i><span class="mobile hidden">新增匯入</span></a>
            </div>
        </div>
    </div>
    <!-- 工具列 End -->
    ...
    <div class="myContentBody">
        ...
        <div class="ui orange attached segment">
            ...
            <div class="ui two column grid">
                <div class="column">
                    <a href="<%=FuncPath() %>?dbs=<%=Req_DBS %>" class="ui small button"><i class="refresh icon"></i>重置條件</a>
                </div>
                    ...
                    <tr>
                        <td class="left aligned collapsing">
                            <b class="red-text text-darken-2">
                                <asp:PlaceHolder ID="ph_ErrLog" runat="server">
                                    <i class="red-text text-darken-3 exclamation triangle icon" title="此筆有錯誤記錄"></i>
                                </asp:PlaceHolder>
                                <%#Eval("TraceID") %></b>
                        </td>
                        ...
                        <td class="left aligned collapsing">
                            <asp:PlaceHolder ID="ph_KeepGo" runat="server">
                                <a class="ui small teal basic icon button" href="<%#fn_Params.WebUrl %>myErpPriceData/ImportStep2.aspx?id=<%#Eval("Data_ID") %>&dbs=<%#Eval("DBS") %>" title="繼續匯入">
                                    <i class="pencil icon"></i>
                                </a>
                            </asp:PlaceHolder>
                            <a class="ui small grey basic icon button" href="<%#fn_Params.WebUrl %>myErpPriceData/View.aspx?id=<%#Eval("Data_ID") %>&dbs=<%#Eval("DBS") %>" title="查看記錄">
                                <i class="file alternate icon"></i>
                            </a>
                            <%--<asp:PlaceHolder ID="ph_Del" runat="server">
                                <asp:LinkButton ID="lbtn_Close" runat="server" CssClass="ui small red basic icon button" ValidationGroup="List" CommandName="doClose" OnClientClick="return confirm('確定作廢?\n執行後無法復原.')" ToolTip="作廢"><i class="trash alternate icon"></i></asp:LinkButton>
                            </asp:PlaceHolder>--%>
