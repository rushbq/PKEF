...
public partial class IT_HelpEdit : SecurityIn
{
    //回覆權限
    ...
    public string MailSenderName = "寶工鋼鐵人";
    public string MailFuncName = "PKEF, 需求登記";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                //[取得/檢查參數] - 問題類型
                if (fn_Extensions.Menu_ITClass(this.ddl_Req_Class, "", true, new List<string> { "登記問題類型" }, out ErrMsg) == false)
                {
            ...
            {
                fn_Extensions.JsAlert("系統發生錯誤！", Page_SearchUrl);
                return;
            }
        }
    }
    ...
    private void View_Data()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("  INNER JOIN IT_ParamClass Cls ON Main.Req_Class = Cls.Class_ID");
                SBSql.AppendLine(" WHERE (TraceID = @TraceID);");

                //若為回覆人員,則Update狀態為處理中(10)
                if (ReplyAuth)
                {
                    SBSql.AppendLine("UPDATE IT_Help SET Help_Status = 10 WHERE (TraceID = @TraceID) AND (Help_Status = 9)");
                }
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        {
                            //導向View頁
                            Response.Redirect("IT_HelpView.aspx?TraceID={0}".FormatThis(Server.UrlEncode(Cryptograph.MD5Encrypt(Param_thisID, DesKey))));
                            return;
                        }
    ...
    private void LookupData_CC(string traceID)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.Append(" FROM IT_Help_CC Base");
                SBSql.Append("  INNER JOIN PKSYS.dbo.User_Profile Prof ON Base.Item_Name = Prof.Account_Name");
                SBSql.Append(" WHERE (Base.TraceID = @DataID) AND (Prof.Display = 'Y')");
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT Custom_ID FROM IT_ParamClass WHERE Class_ID = @Class_ID ");
            SBSql.AppendLine(" );");
            //是否通知主管
            SBSql.AppendLine(" SET @IsNotify = (");
            SBSql.AppendLine("  SELECT Notify FROM IT_ParamClass WHERE Class_ID = @Class_ID ");
            SBSql.AppendLine(" );");
            //New ID
            SBSql.AppendLine(" SET @NewID = (");
            SBSql.AppendLine("  SELECT ISNULL(MAX(CAST(RIGHT(TraceID ,4) AS INT)) ,0) + 1 FROM IT_Help WHERE (SUBSTRING(TraceID,3,8) = @thisDate)");
            SBSql.AppendLine(" );");
        ...
        {
            //發信給資訊人員(IT_Help_Receiver, MailType=1)
            SendMail_Req(NewTraceID, _subject);

            //發送通知信給主管(權限申請)
            if (IsNotify.Equals("Y"))
            {
                SendMail_InfoSupervisor(NewTraceID, _subject, this.ddl_Dept.SelectedValue);
            }

            //新增CC名單, 發信給CC名單
            if (CCMails(NewTraceID))
            {
                SendMail_Req_byCC(NewTraceID, _subject);
            }
    ...
    private bool CCMails(string traceID)
    {
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            foreach (var item in query)
            {
                //-- 產生序號 --
                SBSql.AppendLine(" SET @NewID = (");
                SBSql.AppendLine("  SELECT ISNULL(MAX(Item_ID) ,0) + 1 FROM IT_Help_CC WHERE (TraceID = @TraceID)");
                SBSql.AppendLine(" );");

                //新增資料
                SBSql.Append(" INSERT INTO IT_Help_CC( ");
                ...
                SBSql.Append(" WHERE (Display = 'Y') AND (Email IS NOT NULL) AND (Email <> '') AND (Account_Name = N'{0}'); ".FormatThis(item.Val));
    ...
    protected void btn_Inform_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                StringBuilder SBSql = new StringBuilder();
                SBSql.AppendLine(" UPDATE IT_Help SET Help_Status = 10 ");
    ...
    private void closeCase(bool sendMail)
    {
        try
        {
            ...
            {
                StringBuilder SBSql = new StringBuilder();
                SBSql.AppendLine(" UPDATE IT_Help SET Help_Status = 11 ");
                SBSql.AppendLine("  , Reply_Hours = @Reply_Hours, Reply_Content = @Reply_Content, Reply_Date = @Reply_Date ");
    ...
    protected void btn_onTop_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                StringBuilder SBSql = new StringBuilder();
                SBSql.AppendLine(" UPDATE IT_Help SET onTop = 'Y' ");
                SBSql.AppendLine(" WHERE (TraceID = @TraceID)");
    ...
    /// <param name="OldName">原始顯示檔名</param>
    /// <param name="ReSize">是否顯示放大圖</param>
    /// <returns>string</returns>
    public string PicUrl(string PicName, string OldName, bool ReSize)
    {
        string preView = "";
        ...
        if (PicName.ToLower().IndexOf(".jpg") > 0 || PicName.ToLower().IndexOf(".png") > 0)
        {
            ...
            if (ReSize)
            {
                preView = string.Format(
                   "<a class=\"PicGroup L2Img\" rel=\"PicGroup\" href=\"{0}\"><img src=\"{0}\" border=\"0\" style=\"width:120px\" title=\"{1}\" alt=\"Pic\"></a>"
            ...
            {
                preView = string.Format(
                   "<img src=\"{0}\" border=\"0\" width=\"300\" alt=\"Pic\" style=\"padding-right:5px;\" title=\"{1}\">"
                   , Param_WebFolder + PicName
                   , OldName);
            }
        }
    ...
    private string showAttachments(string TraceID)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                StringBuilder SBSql = new StringBuilder();
                SBSql.AppendLine("SELECT AttachFile, AttachFile_Org FROM IT_Help_Attach WHERE (TraceID = @TraceID)");
                cmd.CommandText = SBSql.ToString();
                cmd.Parameters.AddWithValue("TraceID", TraceID);
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    {
                        showImg.Append("<li>{0}</li>".FormatThis(PicUrl(DT.Rows[row]["AttachFile"].ToString(), DT.Rows[row]["AttachFile_Org"].ToString(), false)));
                    }
                }
            }
    ...
    private void SendMail_Req(string TraceID, string Subject)
    {
        ...
        //[設定參數] - 收件人
        fn_Mail.Reciever = fn_CustomUI.emailReceiver("1");
        ...
        //[設定參數] - 郵件主旨
        fn_Mail.Subject = "[需求登記] {0};追蹤編號:{1}".FormatThis(Subject, convertTraceID(TraceID));
    ...
    private void SendMail_Req_byCC(string TraceID, string Subject)
    {
        ...
        //[設定參數] - 收件人
        fn_Mail.Reciever = fn_CustomUI.emailCC(TraceID);
        ...
        //[設定參數] - 郵件主旨
        fn_Mail.Subject = "[需求登記][轉寄通知] {0};追蹤編號:{1}".FormatThis(Subject, convertTraceID(TraceID));
    ...
    private void SendMail_InfoSupervisor(string TraceID, string Subject, string DeptID)
    {
        ...
        //[設定參數] - 郵件主旨
        fn_Mail.Subject = "[需求登記][核准通知] {0};追蹤編號:{1}".FormatThis(Subject, convertTraceID(TraceID));
        ...
        {
            //Response.Write(fn_Mail.Message);
            fn_Extensions.JsAlert("發生錯誤 - 權限申請發信", "");
            return;
        }
    }
    ...
    private void SendMail_Inform(string TraceID, string Subject, string ReqEMail)
    {
        ...
        //[設定參數] - 轉寄人群組
        fn_Mail.CC = fn_CustomUI.emailCC(TraceID);

        //[設定參數] - 密件轉寄人群組
        fn_Mail.BCC = null;

        //[設定參數] - 郵件主旨
        fn_Mail.Subject = "[需求登記][回覆通知] {0};追蹤編號:{1}".FormatThis(Subject, convertTraceID(TraceID));
    ...
    private void SendMail_Done(string TraceID, string Subject, string ReqEMail)
    {
        ...
        //[設定參數] - 郵件主旨
        fn_Mail.Subject = "[需求登記][結案通知] {0};追蹤編號:{1}".FormatThis(Subject, convertTraceID(TraceID));
        ...
        switch (type)
        {
                    ...
                    using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                    {
                        if (DT.Rows.Count > 0)
                        {
                            ...
                            mailBody.Append("    <a href=\"{0}\" style=\"text-decoration:none;font-weight:bold;\">&raquo;&raquo;&raquo; 知道了，來去處理 &laquo;&laquo;&laquo;</a>".FormatThis(
                                "{0}?t=ITHelp_Reply&dataID={1}".FormatThis(Application["WebUrl"].ToString(), Server.UrlEncode(Cryptograph.MD5Encrypt(traceID, DesKey)))
                                ));
                            mailBody.AppendLine("</div>");
                        }
                    }
            ...
            case "2":
                ...
                mailBody.AppendLine("<a href=\"{0}\" style=\"text-decoration:none;font-weight:bold;\">&raquo;&raquo;&raquo; 請點我前往處理 &laquo;&laquo;&laquo;</a>".FormatThis(
                    "{0}Recording/Req_Process.aspx?dataID={1}".FormatThis(Application["WebUrl"].ToString(), traceID)
                    ));
                mailBody.AppendLine("</div>");
    ...
    public string PageUrl
    {
        get
        {
            return string.Format(@"IT_HelpEdit.aspx?TraceID={0}"
    ...
    public string Page_SearchUrl
    {
        get
        {
            string tempUrl = PKLib_Method.Methods.CustomExtension.getCookie("EF_ITHelp");

            return string.IsNullOrWhiteSpace(tempUrl) ? fn_Params.WebUrl + "Recording/IT_HelpSearch.aspx" : Server.UrlDecode(tempUrl);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null ? this._Param_WebFolder : Application["File_WebUrl"] + @"PKEF/IT_Help/";
        }
        set
        {
