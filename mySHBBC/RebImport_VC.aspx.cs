...
public partial class mySHBBC_Import_VC : SecurityIn
{
    ...
    protected void lbtn_Next_Click(object sender, EventArgs e)
    {
        ...
        {
            string getTraceID = "BT" + NewTraceID();
            ...
            foreach (var gp in groups)
            {
                ...
                if (!string.IsNullOrWhiteSpace(place))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(custID))
                    {
                        ...
                        //(機構=[13])
                        var newDT = queryXls
                                .Where(c => c[13].Cast<string>().Equals(place))
                                .Select(fld => new
                                {
                                    SupName = fld[2],
                                    Qty = fld[7], //销退数量[1]
                                    ...
                                    RebNo = fld[0], //退货单号[5]
                                    EMail = fld[17],
                                    UnitPrice = fld[8], //单价[7]
                                    Addr = fld[14],
                                    Who = fld[15],
                                    RemarkID = fld[18] //單身備註
                                });
