...
public partial class InvoiceList : SecurityIn
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth_User("867", out ErrMsg) == false)
                {
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.GetDataList_SH(search, out ErrMsg);
    ...
    public string PageUrl
    {
        get
        {
            return "{0}mySHBBC/InvoiceList.aspx".FormatThis(Application["WebUrl"]);
        }
        set
        {
            this._PageUrl = value;
