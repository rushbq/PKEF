...
public partial class myTWBBC_Mall_ImportList : SecurityIn
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        if (query.Count() == 0)
        {
            ...
            //Clear
            CustomExtension.setCookie("EF_TWBBC_Mall", "", -1);
        }
        else
        {
            ...
            //暫存頁面Url, 給其他頁使用
            CustomExtension.setCookie("EF_TWBBC_Mall", Server.UrlEncode(reSetPage), 1);
    ...
    public string keepGoUrl(int status, string dataID)
    {
        ...
        {
            case 12:
                return "{0}myTWBBC_Mall/ImportStep4.aspx?id={1}".FormatThis(fn_Params.WebUrl, dataID);

            default:
                return "{0}myTWBBC_Mall/ImportStep2.aspx?id={1}".FormatThis(fn_Params.WebUrl, dataID);
        }
    }
    #endregion
    ...
    {
        return "{0}myTWBBC_Mall/ImportList.aspx".FormatThis(
            fn_Params.WebUrl);
    }
