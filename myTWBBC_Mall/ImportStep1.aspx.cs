...
public partial class myTWBBC_Mall_ImportStep1 : SecurityIn
{
    ...
    {
        string errTxt = "";
        string _mall = ddl_Mall.SelectedValue;
        string _cust = val_Cust.Text;

        //必填檢查
        if (string.IsNullOrWhiteSpace(_cust) || string.IsNullOrWhiteSpace(_mall))
        {
            errTxt += "===請檢查以下欄位===\\n";
            errTxt += "商城\\n";
            errTxt += "客戶\\n";
    ...
    private string[] Add_Data()
    {
        ...
        * 必須上傳2個檔案(訂單/出貨)
        */
        if (_mall.Equals("1") && fileCnt < 2)
        {
            CustomExtension.AlertMsg("商城=好市多,必須上傳訂單與出貨檔", "");
            return new string[] { DataID, ProcCode, Message };
        }
        #endregion
        ...
        var data = new ImportData
        {
            ...
            TraceID = myTraceID,
            CustID = _cust,
            MallID = Convert.ToInt16(_mall),
            Upload_File = ITempList[0].Param_FileName,
            Upload_ShipFile = ITempList.Count == 2 ? ITempList[1].Param_FileName : "",
