...
public partial class redirect : SecurityIn
{
        ...
        try
        {
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    if (!resp.Create_ClickInfo(3, Convert.ToInt32(Req_MenuID), fn_Params.UserGuid, out ErrMsg))
                    {
    ...
    public string Req_Url
    {
        get
        {
            String _data = Request.QueryString["url"] == null ? fn_Params.WebUrl : Request.QueryString["url"];
            return _data;
        }
        set
        {
