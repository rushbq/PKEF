...
public partial class Dept_Search : SecurityIn
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 部門
                if (fn_Extensions.Menu_DeptForTarget(this.ddl_Dept, Request.QueryString["Dept"], true, out ErrMsg) == false)
                {
                ...
                #region "--細項權限判斷--"
                if (fn_CheckAuth.CheckAuth_User("102", out ErrMsg))
                {
                    //[取得物件] - 判斷資料列表
    ...
    private void LookupDataList(int page)
    {
        ...
        try
        {
            ...
            SBSql.AppendLine("      Shipping.SName, User_Dept.DeptName, TarData.ShipFrom, TarData.DeptID, TarData.SetYear ");
            ...
            SBSql.AppendLine("      , SUM(TarData.ChAmount_NTD) AS ChAmount_NTD, SUM(TarData.ChAmount_USD) AS ChAmount_USD, SUM(TarData.ChAmount_RMB) AS ChAmount_RMB ");
            ...
            SBSql.AppendLine(" FROM ( ");
            SBSql.AppendLine("  SELECT TarData.ShipFrom, TarData.DeptID, TarData.SetYear ");
            SBSql.AppendLine("  FROM Target_Dept TarData ");
            SBSql.AppendLine("      INNER JOIN PKSYS.dbo.Shipping ON TarData.ShipFrom = Shipping.SID ");
            SBSql.AppendLine("      INNER JOIN PKSYS.dbo.User_Dept ON TarData.DeptID = User_Dept.DeptID ");
            SBSql.AppendLine("  WHERE (1 = 1) ");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    string Get_SetYear = ((HiddenField)e.Item.FindControl("hf_SetYear")).Value;

                    //[SQL] - 刪除資料
                    SBSql.Clear();
                    SBSql.AppendLine(" DELETE FROM Target_Dept WHERE (ShipFrom = @ShipFrom) AND (DeptID = @DeptID) AND (SetYear = @SetYear) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("ShipFrom", Get_ShipFrom);
                    cmd.Parameters.AddWithValue("DeptID", Get_DeptID);
                    cmd.Parameters.AddWithValue("SetYear", Get_SetYear);
