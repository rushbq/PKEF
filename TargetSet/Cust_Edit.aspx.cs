...
public partial class Cust_Edit : SecurityIn
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                //[權限判斷] - 客戶目標 - 編輯
                if (fn_CheckAuth.CheckAuth_User("121", out ErrMsg) == false && fn_CheckAuth.CheckAuth_User("122", out ErrMsg) == false)
                {
                    Response.Redirect(string.Format("../Unauthorized.aspx?ErrMsg={0}", HttpUtility.UrlEncode(ErrMsg)), true);
                    return;
                }
                //無:管理者權限，有:一般權限 -> 只顯示自己的客戶
                ...
                if (false == string.IsNullOrEmpty(CustID))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        StringBuilder SBSql = new StringBuilder();

                        SBSql.AppendLine("SELECT RTRIM(MA001) AS CustID, MA002 AS CustName ");
                        SBSql.AppendLine(" FROM Customer WITH (NOLOCK) ");
                        SBSql.AppendLine(" WHERE (DBS = DBC) AND (RTRIM(MA001) = @CustID) ");
                        ...
                        using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.PKSYS, out ErrMsg))
                        {
                            if (DT.Rows.Count > 0)
                            {
                                this.tb_CustName.Text = DT.Rows[0]["CustName"].ToString();
                                this.tb_CustID.Text = DT.Rows[0]["CustID"].ToString();
                                this.lb_CustID.Text = DT.Rows[0]["CustID"].ToString();
                            }
                        }
                    }
                }
                ...
                {
                    this.lt_MessageTxt.Text = "Step 2. 填寫客戶目標金額";
                }
    ...
    private void View_Data()
    {
        try
        {
            ...
            {
                StringBuilder SBSql = new StringBuilder();
                SBSql.AppendLine(" SELECT UID, ShipFrom, CustID, SetYear, SetMonth, Customer.MA002 AS CustName ");
                SBSql.AppendLine("  , Amount_NTD, Amount_USD, Amount_RMB");
                SBSql.AppendLine("  , OrdAmount_NTD, OrdAmount_USD, OrdAmount_RMB");
                SBSql.AppendLine(" FROM Target_Customer INNER JOIN PKSYS.dbo.Customer ");
                SBSql.AppendLine("  ON Target_Customer.CustID = Customer.MA001 AND Customer.DBC = Customer.DBS ");
                SBSql.AppendLine(" WHERE (Target_Customer.UID = @UID) ");
                cmd.CommandText = SBSql.ToString();
                cmd.Parameters.Clear();
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.EFLocal, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.tb_CustID.Text = DT.Rows[0]["CustID"].ToString().Trim();
                        this.tb_CustName.Text = DT.Rows[0]["CustName"].ToString().Trim();
                        ...
                        //顯示提醒文字
                        this.lt_MessageTxt.Text = "修改客戶目標金額";
                    }
                }
            }
        }
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //[SQL] - 刪除資料
                    SBSql.Clear();
                    SBSql.AppendLine(" DELETE FROM Target_Customer WHERE (UID = @Param_ID) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("Param_ID", GetDataID);
    ...
    private void Add_Data()
    {
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 取得最新編號
            int New_ID;
            SBSql.AppendLine(" SELECT (ISNULL(MAX(UID), 0) + 1) AS New_ID FROM Target_Customer ");
            cmd.CommandText = SBSql.ToString();
            using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.EFLocal, out ErrMsg))
            {
    ...
    private void Edit_Data()
    {
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.Clear();
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Target_Customer ");
            SBSql.AppendLine(" SET ShipFrom = @ShipFrom, CustID = @CustID, SetYear = @SetYear, SetMonth = @SetMonth");
    ...
    protected void btn_showDT_Click(object sender, EventArgs e)
    {
        Response.Redirect(PageUrl_byYear);
    }
    #endregion
