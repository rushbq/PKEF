...
public partial class Cust_Search : SecurityIn
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                #region "--細項權限判斷--"
                if (fn_CheckAuth.CheckAuth_User("121", out ErrMsg) || fn_CheckAuth.CheckAuth_User("122", out ErrMsg))
                {
                    //[取得物件] - 判斷資料列表
                    if (this.lvDataList.Items.Count > 0)
                    {
                        ...
                        for (int i = 0; i < lvDataList.Items.Count; i++)
                        {
                            ...
                            //若為使用者編輯, 判斷是否為自己的客戶
                            HiddenField hf_IsRel = ((HiddenField)this.lvDataList.Items[i].FindControl("hf_IsRel"));
                            if (hf_IsRel.Value.Equals("0") && fn_CheckAuth.CheckAuth_User("121", out ErrMsg) == false)
                            {
    ...
    private void LookupDataList(int page)
    {
        ...
        try
        {
            ...
            SBSql.AppendLine("      , RTRIM(Customer.MA001) AS CustID, RTRIM(Customer.MA002) AS CustName ");
            ...
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY TarData.SetYear DESC, TarData.ShipFrom, Customer.MA001) AS RowRank ");
            
            //判斷是否為自己或屬下的客戶
            SBSql.Append(" , (");
            SBSql.Append(" SELECT COUNT(*) FROM PKSYS.dbo.Staff_Rel_Customer Rel");
            SBSql.Append(" WHERE (Rel.CustID = TarData.CustID) AND ((Rel.StaffID = @currUser)");
            SBSql.Append("   OR EXISTS (");
            SBSql.Append("    SELECT Prof.Account_Name FROM PKSYS.dbo.User_Profile Prof INNER JOIN PKSYS.dbo.User_Dept_Supervisor Sup ON Prof.DeptID = Sup.DeptID");
            SBSql.Append("    WHERE (Prof.Account_Name = Rel.StaffID) AND (Sup.Account_Name = @currUser)");
            SBSql.Append("   )");
            SBSql.Append("  )");
            SBSql.Append(" ) AS IsRel");
            ...
            {
                SBSql.Append(" AND ( ");
                SBSql.Append("      (Customer.MA001 LIKE '%' + @Param_Keyword + '%') ");
                SBSql.Append("   OR (Customer.MA002 LIKE '%' + @Param_Keyword + '%') ");
                SBSql.Append("   OR (Customer.MA003 LIKE '%' + @Param_Keyword + '%') ");
                SBSql.Append(" ) ");
            ...
            SBSql.AppendLine("    GROUP BY Shipping.SName, TarData.ShipFrom, TarData.SetYear, TarData.CustID, Customer.MA001, Customer.MA002 ");
            ...
            SBSql.AppendLine(" FROM ( ");
            SBSql.AppendLine("    SELECT ");
            SBSql.AppendLine("      Shipping.SName, TarData.ShipFrom, TarData.SetYear, Customer.MA001 ");
            SBSql.AppendLine("    FROM Target_Customer TarData ");
            SBSql.AppendLine("      INNER JOIN PKSYS.dbo.Shipping ON TarData.ShipFrom = Shipping.SID ");
            SBSql.AppendLine("      INNER JOIN PKSYS.dbo.Customer ON TarData.CustID = Customer.MA001 AND Customer.DBC = Customer.DBS ");
            SBSql.AppendLine("    WHERE (TargetType = @TargetType)");
            ...
            SBSql.AppendLine("    GROUP BY Shipping.SName, TarData.ShipFrom, TarData.SetYear, TarData.CustID ");
            SBSql.AppendLine("      , Customer.MA001, Customer.MA002 ");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //[SQL] - 刪除資料
                    SBSql.Clear();
                    SBSql.AppendLine(" DELETE FROM Target_Customer WHERE (ShipFrom = @ShipFrom) AND (CustID = @CustID) AND (SetYear = @SetYear) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
