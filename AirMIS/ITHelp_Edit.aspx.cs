...
public partial class AirMIS_ITHelp_Edit : SecurityIn
{
    //設定FTP連線參數
    private FtpMethod _ftp = new FtpMethod(
        fn_Params.ftp_Username, fn_Params.ftp_Password, fn_Params.ftp_ServerUrl);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                Get_ClassList("B", ddl_ReqClass, "請選擇", ""); //需求類別
                ...
                if (string.IsNullOrWhiteSpace(Req_DataID))
                {
                    ...
                    //需求者(目前使用者)
                    string _accID = fn_Params.UserAccount;
                    filter_Emp.Text = _accID;
                    lb_Emp.Text = fn_Params.UserAccountName;
                    val_Emp.Text = _accID;
                }
                else
                {
    ...
    private void LookupData()
    {
        ...
        try
        {
            ...
            //結案權限判斷
            if (_currStatus.Equals("125") && !_ReplyAuth)
            {
                //無權限不能繼續編輯
    ...
    protected void lv_Attachment_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            //----- 方法:刪除資料 -----
            if (false == _data.Delete_ITHelpFiles(Get_DataID))
            {
    ...
    protected void lv_ProcList_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            //----- 方法:刪除資料 -----
            if (false == _data.Delete_ITHelpProcItem(_ParentID, _DetailID))
            {
                CustomExtension.AlertMsg("檔案刪除失敗", "");
                return;
            }
    ...
    protected void btn_doSaveBase_Click(object sender, EventArgs e)
    {
        ...
        //申請類別=專案,檢查
        if (rbl_ApplyType.SelectedValue.Equals("3") && string.IsNullOrWhiteSpace(tb_Help_Benefit.Text.Trim()))
        {
            errTxt += "請填寫「改善效益 」\\n";
        }
    ...
    private void Add_Data(List<IOTempParam> fileList, string guid, string traceID)
    {
        ...
        var data = new ItHelpData
        {
            ...
            TraceID = traceID,
            Apply_Type = Convert.ToInt16(rbl_ApplyType.SelectedValue),
        ...
        //----- 方法:新增資料 -----
        if (!_data.CreateITHelp_Base(data, out ErrMsg))
        {
        ...
        {
            //----- 宣告:資料參數 -----
            List<ITHelpAttachment> dataItems = new List<ITHelpAttachment>();

            //----- 設定:資料欄位 -----
            for (int row = 0; row < fileList.Count; row++)
            {
                var dataItem = new ITHelpAttachment
                {
            ...
            //----- 方法:更新資料 -----
            if (false == _data.CreateITHelp_Attachment(guid, "", "A", dataItems, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
        ...
        if (!string.IsNullOrWhiteSpace(infomWho))
        {
            ...
            //----- 宣告:資料參數 -----
            List<ITHelpCC> dataItems = new List<ITHelpCC>();

            //----- 設定:資料欄位 -----
            foreach (var item in query)
            {
                var dataItem = new ITHelpCC
                {
                    CC_Who = item.who
                };
            ...
            //----- 方法:更新資料 -----
            if (false == _data.CreateITHelp_Inform(guid, dataItems, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
    ...
    private void Edit_Data(List<IOTempParam> fileList, string guid)
    {
        ...
        //----- 方法:修改資料 -----
        if (!_data.Update_ITHelpBase(data, out ErrMsg))
        {
            this.ph_ErrMessage.Visible = true;
    ...
    protected void btn_doSaveReply_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //----- 方法:修改資料 -----
            if (!_data.Update_ITHelpReply(data, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
    ...
    protected void btn_doSaveProc_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //----- 方法:修改資料 -----
            _newDetailID = _data.CreateITHelp_Proc(_id, data, out ErrMsg);
    ...
    private void doProcUpload(string _id, string _detailID, string _traceID)
    {
        ...
        if (ITempList.Count > 0)
        {
            ...
            try
            {
                ...
                //----- 方法:更新資料 -----
                if (false == _data.CreateITHelp_Attachment(_id, _detailID, "B", dataItems, out ErrMsg))
                {
                    CustomExtension.AlertMsg("新增檔案失敗", "");
                }
            }
    ...
    protected void btn_Finish_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //----- 方法:修改資料 -----
            if (!_data.Update_ITHelpClose(data, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
    ...
    protected void lbtn_doTrace_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (!_data.Update_ITHelpSetTop(Req_DataID, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
    ...
    protected void btn_Inform_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (_type.Equals("B"))
            {
                if (!_data.Update_ITHelpStatus(_id, "C"))
                {
                    this.ph_ErrMessage.Visible = true;
    ...
    private string Get_CurrentStatus(string id)
    {
        ...
        try
        {
            return _data.GetOne_ITHelpStatus(id);
        }
        catch (Exception)
        {
    ...
    private void Get_ClassList(string clsType, DropDownList ddl, string rootName, string inputValue)
    {
        ...
        //----- 原始資料:取得所有資料 -----
        DataTable query = _data.GetClass_IThelp(clsType, out ErrMsg);
    ...
    private bool doSendInformMail(string _sendType, string _guid, string _customInfo, out string ErrMsg)
    {
        ...
        //[資料取得] - 基本資料
        var baseData = _data.GetOne_IThelp(search, out ErrMsg).Take(1).FirstOrDefault();
        if (baseData == null)
        {
            ErrMsg = "查無資料";
            return false;
    ...
    private ArrayList Get_MailList(string sendType, string dataID, string reqEmail, string reqDeptID)
    {
        ...
        switch (sendType)
        {
            case "A":
                ...
                var dataA = _data.GetITHelpReceiver("1");
            ...
            case "B":
                //轉寄通知:CC設定人員
                var dataB = _data.GetITHelpCCList(dataID);
            ...
            case "D":
                //結案:固定收件人(新需求=1 / 結案=2)
                var dataD = _data.GetITHelpReceiver("2");
                ...
                dataD = null;
                //轉寄人員
                var dataD1 = _data.GetITHelpCCList(dataID);
    ...
    /// <returns></returns>
    private StringBuilder Get_MailContent(string sendType, string guid, string _customInfo, ItHelpData baseData)
    {
        //宣告
        ...
        //Html模版路徑(From CDN)
        string url = "{0}PKEF/ITHelp/Mail_TW.html?v=1.0".FormatThis(fn_Params.CDNUrl);
        ...
        //[取代指定內容]:郵件固定內容
        string msg = "";
        string editUrl = "{0}ITHelp_Edit.aspx?id={1}".FormatThis(FuncPath(), guid);
        string viewUrl = "{0}ITHelp_View.aspx?id={1}".FormatThis(FuncPath(), guid);
        string pageUrl = "";
        switch (sendType)
        {
            case "A":
                msg = "<p>目前有新的資訊需求，請前往處理。</p><p>需求類別依內容判斷後，請於「回覆資料」區塊修改。</p>";
        ...
        //工時欄位display
        //html.Replace("#Disp1#", baseData.Help_Status.Equals(125) ? "" : "display:none");
        ...
        //[資料取得] - 附件清單
        var fileData = _data.GetITHelpFileList(guid, "", "A");
    ...
    {
        return "{0}AirMIS/".FormatThis(fn_Params.WebUrl);
    }
    #endregion
